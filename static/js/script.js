document.addEventListener('DOMContentLoaded', () => {
  ...
  let baseLabel = 'All datasets';  // shown in the blue banner as the source
  ...
  const btnPhase34 = document.getElementById('btn-apply-phase3'); // button text is "Run Phase 3&4"
  ...
  async function fetchData() {
    try {
      ...
      // Accept either processed array or raw object with "rows"
      ...
      baseDatasets = allDatasets.slice(); // default base
      initialize();
    } catch (err) {
  ...
  function populateFilters() {
    const dimensions = uniq(allDatasets.flatMap(d => d.dimension));
    const modalities = uniq(allDatasets.flatMap(d => d.modality));
    const tasks      = uniq(allDatasets.flatMap(d => d.task));
    populateSelect(dimensionFilter, dimensions);
  ...
  function applyFilters() {
    ...
    currentFilteredData = source.filter(d => {
      ...
      const matchesDimension = !selectedDimension || d.dimension.includes(selectedDimension);
    ...
    const anyQuick = !!searchTerm || !!selectedDimension || !!selectedModality || !!selectedTask;
  ...
  function setupEventListeners() {
    ...
    btnPhase12?.addEventListener('click', () => {
      ...
      // base for table becomes Phase 1&2 result
    ...
    btnPhase34?.addEventListener('click', () => {
      ...
      // base for table becomes Phase 3 result
  ...
  function renderDimensionChart(datasets) {
    ...
    createOrUpdateDonutChart(canvasId, labels, data, dimensionFilter, { compact: false });
  }
  ...
  function createOrUpdateBarChart(canvasId, labels, data) {
    ...
    // fixed compact height (CSS also constrains height)
    canvas.style.maxHeight = '220px';
    canvas.style.height = '220px';
